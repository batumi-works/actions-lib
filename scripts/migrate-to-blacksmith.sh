#!/usr/bin/env bash
# Migrate GitHub Actions workflows to use Blacksmith runners for organization repos
...
# Blacksmith runner mapping
declare -A RUNNER_MAP=(
    ["ubuntu-latest"]="blacksmith-2vcpu-ubuntu-2204"
    ["ubuntu-22.04"]="blacksmith-2vcpu-ubuntu-2204"
    ["ubuntu-20.04"]="blacksmith-2vcpu-ubuntu-2204"
    ["ubuntu-18.04"]="blacksmith-2vcpu-ubuntu-2204"
)
    ...
    while IFS= read -r line; do
        ...
        if [[ "$line" =~ ^[[:space:]]*runs-on:[[:space:]]* ]]; then
            ...
            if [[ -n "${RUNNER_MAP[$current_runner]:-}" ]]; then
                ...
                echo "${indent}runs-on: $new_runner # Blacksmith runner (fallback: $current_runner)" >> "$temp_file"
...
add_fallback_template() {
    ...
    local template_file="$template_dir/blacksmith-fallback.yml"
...
name: 'Blacksmith Runner with Fallback'

on:
  workflow_dispatch:

jobs:
  # Primary job on Blacksmith runner
  build-primary:
    ...
    steps:
      ...
      - name: Build
        id: build
        run: |
          echo "Building on Blacksmith runner..."
  ...
  build-fallback:
    ...
    steps:
      - name: Fallback Warning
        run: |
          echo "::warning::Blacksmith runner unavailable, using GitHub-hosted runner"
...
# Main execution
main() {
    print_color "$BLUE" "=== Blacksmith Runner Migration Tool ==="
    print_color "$YELLOW" "This tool will migrate your workflows to use Blacksmith runners"
    echo ""
    ...
    if [[ ! "$repo_url" =~ batumi-works ]]; then
        print_color "$YELLOW" "Warning: This doesn't appear to be a batumi-works organization repository"
    ...
    print_color "$GREEN" "✓ Workflows migrated to Blacksmith runners"
